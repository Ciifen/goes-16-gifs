...
def getScaleOffset(path):
    ...
    scale = nc.variables['RRQPE'].scale_factor
    offset = nc.variables['RRQPE'].add_offset
...
def remap(paths, extent, resolution, driver):
    # Initialize an empty array to store the accumulated precipitation
    ...
    for path in paths:
        ...
        if(driver == 'NETCDF'):
            connectionInfo = 'NETCDF:\"' + path + '\":RRQPE'
        else: # HDF5
            connectionInfo = 'HDF5:\"' + path + '\"://RRQPE'
        ...
        if accumulated_precipitation is None:
            accumulated_precipitation = array
        else:
            accumulated_precipitation += array

    # Set nodata value
    accumulated_precipitation[np.isnan(accumulated_precipitation)] = -1

    # Create a new raster to store the accumulated precipitation
    grid_accumulated = memDriver.Create('accumulated_precipitation', sizex, sizey, 1, gdal.GDT_Float32)

    # Setup projection and geo-transformation
    grid_accumulated.SetProjection(targetPrj.ExportToWkt())
    grid_accumulated.SetGeoTransform(getGeoT(extent, grid_accumulated.RasterYSize, grid_accumulated.RasterXSize))

    # Write accumulated precipitation to the raster
    grid_accumulated.GetRasterBand(1).WriteArray(accumulated_precipitation)

    return grid_accumulated
