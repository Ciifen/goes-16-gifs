  ...
  for path in paths:
    ...
    for indice,pais in enumerate(paises):
      ...
      if(indice ==0):
        ...
        plt.savefig('/var/py/volunclima/scripts/goes-16/Output/14/ECUADOR/CMIPF_Band_14_'+date+'_'+time+'_ECUADOR.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
      elif(indice ==1):
        ...
        plt.savefig('/var/py/volunclima/scripts/goes-16/Output/14/GUAYAQUIL/CMIPF_Band_14_'+date+'_'+time+'_GUAYAQUIL.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
      else:
        ax.figimage(logo_GNC, 10, 55, zorder=3, alpha = 1, origin = 'upper')
        ax.figimage(logo_INPE, 400, 55, zorder=3, alpha = 1, origin = 'upper')
        ax.figimage(logo_NOAA, 500, 55, zorder=3, alpha = 1, origin = 'upper')
        ax.figimage(logo_GOES, 585, 55, zorder=3, alpha = 1, origin = 'upper')
        plt.savefig('/var/py/volunclima/scripts/goes-16/Output/14/GALAPAGOS/CMIPF_Band_14_'+date+'_'+time+'_GALAPAGOS.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
      plt.close(ax)
...
for indice,directory in enumerate(directorios):
  ...
  if(indice==0):
    ...
    output_file = directory+'band_14_'+rangoi+'_'+rangof+'_ecuador.gif'
  elif(indice==1):
    # Especificar el nombre del archivo de salida del GIF
    output_file = directory+'band_14_'+rangoi+'_'+rangof+'_guayaquil.gif'
  else:
    output_file = directory+'band_14_'+rangoi+'_'+rangof+'_galapagos.gif'
  frame_duration = 0.7
